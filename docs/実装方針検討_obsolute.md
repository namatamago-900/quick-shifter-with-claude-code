...
**※ [実装方針検討.md](./実装方針検討.md)が本ファイルをモジュール化したものなのでobsoluteにするが念のため残しておく。**
...
- [4. 安全設計](#4-安全設計)
- [5. ハードウェア要件（pin_assign.md への差分)](#5-ハードウェア要件pin_assignmd-への差分)
- [6. スコープ外・将来拡張](#6-スコープ外将来拡張)
- [7. チューニング手順(実装完了後)](#7-チューニング手順実装完了後)
...
ギア推定ずれ解析（§2.5.3）のため、シフトイベント時は **シフト前/後の rpm と比率** を残す：
...
### 6.2 将来拡張余地
- **ギア推定ずれの自動検知**：§2.5.3 のログを走行中に自動判定して ERROR 表示
