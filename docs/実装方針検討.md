...
`gear == 0`（N）と `gear == 6` は計算経路に入らない（§3.2 抑制条件で阻止）。
...
- **限界**：1→N→2 のようなNを経由した飛びシフトは推定がずれる。`gear.md` §5 のニュートラルスイッチは「1→N誤シフト検出に有効」とされているが、本実装ではこのケースで `currentGear` が一時的に 0→1 を経由するため、次の QSシフトで誤推定する可能性がある（実走チューニングで観測）
- **回復**：N を一度経由すれば再同期する
...
`currentGear == 6` のときシフトSWが押されても、シフトイベントとして処理しない（カウントもカット出力もしない）。

#### 2.5.4 ギア推定ずれの観測（自動補正はスコープ外）
...
#### DEBUG_MODE 時の WDT 注意

§4.3 の WDT は 1秒。115200 bps で TXバッファ満杯 → `Serial.println` ブロッキング → 1秒以内に解消しなければ WDT リセット、という連鎖が起きうる。
