...
VS Code + **Arduino Community Edition** 拡張（`vscode-arduino-community.vscode-arduino`）で開発する。エントリーポイントは `quick-shifter-with-claude-code.ino`。ボードは Arduino Nano（`arduino:avr:nano`）。

コンパイル・アップロードは VS Code のコマンドパレットから `Arduino: Upload` を実行する。Arduino CLI が別途インストールされている場合：
